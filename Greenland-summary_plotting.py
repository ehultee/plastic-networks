...
for out in output_dicts:
    ...
    for j, gl in enumerate(glacier_names):
        ...
        if len(out[gl]) >4:
            true_len = int(0.5*len(out[gl])) #fixing Kanger double-writing bug present in early June.  Will be fixed in newer results
            branch_fx = branch_fx[0:true_len]
...
scenarios = ['CB', 'WB', 'CP', 'WP', 'WX']
##styles = [':', '-.', '--', '-']
markers = ['o', '.', ',', '^', 'd', '*']
styles = ['-', ':', '-.', '-', '-', '-']
cmap = matplotlib.cm.get_cmap('winter')
scenario_colors = matplotlib.cm.get_cmap('Blues')([0.1, 0.3, 0.5, 0.7, 0.9])
...
for j in range(len(perscenario_SLE)):
    plt.plot(ty_50a, perscenario_SLE[j], color=scenario_colors[j], label=scenarios[j])
