...
lb_data = np.loadtxt('/Users/lizz/Desktop/Hindcasted_networks/lowerbound_termini.csv',delimiter=',',skiprows=1)
...
# for glacier_id in glaciers_to_plot:
    ...
    if sum(f1)>2:
        ...
        m3,b,rvalue,pvalue3,err3=linregress(lb_data[f3,1],lb_data[f3,2])
        ...
        if pvalue2>0.05:
            ...
            if ~np.isnan(m1):
                ...
                dLdt_lb.append(m3);dLdt_lb_err.append(err2)
...
dLdt_lb = np.array(dLdt_lb)
dLdt_lb_err = np.array(dLdt_lb_err)
...
plt.errorbar(dLdt_obs,dLdt_lb,xerr=dLdt_obs_err,yerr=dLdt_lb_err,color='k',fmt='D',ecolor='lightgray',capsize=5)
